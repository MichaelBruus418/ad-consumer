...
class BannerService @Inject()(ws: WSClient) {
  ...
  def getBanner(zone: Zone): Future[Option[Banner]] = {
    ...
    val data = Json.obj("publisher" -> "Jyllands-Posten", "zone" -> zone.toString.toLowerCase(), "includeHtml" -> true)

    val eventualBannerOpt = ws.asScala().url("http://localhost:9100/api/creative/request")
